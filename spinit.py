...
def write_config(data):
    ...
    conf = nginx_app_file.format(ports=port, name='registrar', components='')
...
""".format(name=cname.lower(), port=port)
    ...
    return service_check, component_service_map
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate nginx configs and content from data yaml')
    ...
    with open(opts.data, 'r') as stream:
        try:
            ...
            print(exc)
    service_check, component_service_map = write_config(data)
    create_registrar(service_check, component_service_map)
    create_services(service_check, component_service_map)
    print('Index URL: http://127.0.0.1:%s/' % service_check['registrar'][0])
